...
export async function sendEmailOtp(to, code) {
  try {
    ...
    await transporter.sendMail({
      from: `"SecureRecover" <${process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER}>`,
      to,
...
// ارسال OTP از طریق SMS (برای تولید فعلاً mock)
export async function sendSmsOtp(to, code) {
  // در حال حاضر فقط لاگ می‌کنیم (برای پیاده‌سازی واقعی نیاز به سرویس SMS دارید)
  console.log(`[SMS] Sending code ${code} to ${to}`);
  
  // می‌توانید از سرویس‌هایی مثل Kavenegar، Twilio، یا Melipayamak استفاده کنید
  // مثال برای Kavenegar:
  /*
...
export async function sendSecurityAlert(email, action, ip, device) {
  try {
    ...
    await transporter.sendMail({
      from: `"SecureRecover Security" <${process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER}>`,
      to: email,
